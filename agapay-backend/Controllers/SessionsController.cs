...
namespace agapay_backend.Controllers
{
  ...
  public class SessionsController : ControllerBase
  {
    ...
    public async Task<IActionResult> CreateSession(CreateSessionDto dto)
    {
      ...
      // Validate contract exists and matches patient + therapist and is active and within date range
      var contract = await _db.Contracts.FirstOrDefaultAsync(c => c.Id == dto.ContractId);
      if (contract is null) return BadRequest(new { message = "Invalid contract" });
      if (contract.PatientId != patient.Id || contract.PhysicalTherapistId != dto.TherapistId)
        return BadRequest(new { message = "Contract does not match patient / therapist" });
      if (contract.Status != ContractStatus.Active)
        return BadRequest(new { message = "Contract not active" });
      if (dto.StartAt < contract.StartDate || dto.EndAt > contract.EndDate)
        return BadRequest(new { message = "Session outside contract period" });
